.. meta::
   :description: 《数据库实用入门》，一本开放教育资源教科书。
   :keywords: sql, database, relational database, computer science

.. _static/library_small.jpg

=====================================
数据库实用入门
=====================================

.. raw:: html

    <h3>Christopher Painter&#8209;Wakefield, Colorado School of Mines</h3>

原文参考: `<https://runestone.academy/ns/books/published/practical_db/index.html?mode=browsing>`_

目录
:::::::::::::::::

.. toctree::
   :maxdepth: 1

   front-matter/preface

.. toctree::
   :numbered:
   :maxdepth: 2

   PART1_SQL/index
   PART2_DATA_MODELING/index
   PART3_RELATIONAL_DATABASE_THEORY/index

.. toctree::
   :maxdepth: 1

   references/references

.. toctree::
   :maxdepth: 2

   Appendix A: 本书使用的示例数据集 <appendix-a-datasets/datasets>
   Appendix B: SQL 参考 <appendix-b-reference/reference>

* :ref:`genindex`
* :ref:`search`

.. toctree::
   :maxdepth: 1

   front-matter/acknowledgments

封面照片
-----------

**Cover photo**

.. md-tab-set::

   .. md-tab-item:: 中文

      圣三一学院图书馆照片 `Henry Be <https://unsplash.com/@henry_be?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText>`_ 在 `Unsplash <https://unsplash.com/s/photos/library?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText>`_.

   .. md-tab-item:: 英文

      Photo of the library of Trinity College by `Henry Be <https://unsplash.com/@henry_be?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText>`_ on `Unsplash <https://unsplash.com/s/photos/library?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText>`_.

许可证
-------

**License**

.. md-tab-set::

   .. md-tab-item:: 中文

      .. raw:: html

         <div typeof="cc:Work" xmlns:cc="http://creativecommons.org/ns#" xmlns:dct="http://purl.org/dc/terms/">
            <a rel="license" href="http://creativecommons.org/licenses/by-sa/4.0/" target="_blank">
               <img alt="知识共享许可协议" style="display: block;" src="https://i.creativecommons.org/l/by-sa/4.0/88x31.png"/>
            </a>
            <br/>
            <span href="http://purl.org/dc/dcmitype/InteractiveResource" property="dct:title" rel="dct:type">
            <i>数据库实用入门</i>
            </span>
            作者
            <span property="cc:attributionName">Christopher Painter&#8209;Wakefield</span>
            授权于
            <a rel="license" href="http://creativecommons.org/licenses/by-sa/4.0/">知识共享署名-相同方式共享 4.0 国际许可协议</a>.
         </div>

   .. md-tab-item:: 英文

      .. raw:: html

         <div typeof="cc:Work" xmlns:cc="http://creativecommons.org/ns#" xmlns:dct="http://purl.org/dc/terms/">
            <a rel="license" href="http://creativecommons.org/licenses/by-sa/4.0/" target="_blank">
               <img alt="知识共享许可协议" style="display: block;" src="https://i.creativecommons.org/l/by-sa/4.0/88x31.png"/>
            </a>
            <br/>
            <span href="http://purl.org/dc/dcmitype/InteractiveResource" property="dct:title" rel="dct:type">
            <i>A Practical Introduction to Databases</i>
            </span>
            by 
            <span property="cc:attributionName">Christopher Painter&#8209;Wakefield</span>
            is licensed under a
            <a rel="license" href="http://creativecommons.org/licenses/by-sa/4.0/">Creative Commons Attribution-ShareAlike 4.0 International License.</a>.
         </div>

版本
-------

**Version**

.. md-tab-set::

   .. md-tab-item:: 中文

      这是《数据库实用入门》的 0.1.2 版本，提交于 2023-08-26。

   .. md-tab-item:: 英文

      This is release 0.1.2 of A Practical Introduction to Databases, committed 2023-08-26.